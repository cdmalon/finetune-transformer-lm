...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_file', type=str, default='save/best_params.jl')
    parser.add_argument('--test_prefix', type=str, default='data/test')
    parser.add_argument('--result_file', type=str, default='submission/newresult.tsv')
    ...
    if args.dataset == 'rocstories':
        raise ValueError("Not supported by this script")
    elif args.dataset == 'entailment':
        ...
        n_special = 3
        max_len = n_ctx // 2 - 2
        # Take the max over combined sentence lengths for all examples.
        # Take the smaller of that max (+ 3 for special tokens) and min_ctx.
        
        # n_ctx = min(max([len(x1[:max_len])+len(x2[:max_len]) for x1, x2 in zip(tst_p, tst_h)])+3, n_ctx)  # No - we have to use the same value from training
        ...
        # what are dimensions 2 and 4?
        # dimension 2: each alternative.  there's only one in entailment.
        # dimension 4: [,0] is the sequence; [,1] is position.
        # The arange begins above vocab size so we need just one lookup table.
